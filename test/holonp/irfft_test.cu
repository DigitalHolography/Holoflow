...
class IRFFTInferTest : public ::testing::Test {
protected:
  holonp::IRFFTFactory factory;
};

TEST_F(IRFFTInferTest, ExpandsTransformAxisAndOutputsReal) {
  const std::vector<TDesc> in = {device_desc({2, 4}, DType::CF32)};
  const auto               r  = factory.infer(in, nlohmann::json{{"axis", 1}});
  EXPECT_EQ(r.kind, TaskKind::Sync);
  EXPECT_EQ(r.output_descs[0].shape, (std::vector<size_t>{2, 6}));
  EXPECT_EQ(r.output_descs[0].dtype, DType::F32);
}

TEST_F(IRFFTInferTest, ExplicitLengthSupportsOddOutput) {
  const std::vector<TDesc> in = {device_desc({2, 4}, DType::CF32)};
  const auto               r  = factory.infer(in, nlohmann::json{{"axis", 1}, {"n", 5}});
  EXPECT_EQ(r.output_descs[0].shape, (std::vector<size_t>{2, 5}));
}

TEST_F(IRFFTInferTest, RejectsRealInput) {
...
class IRFFTOracleTest : public ::testing::Test {
protected:
  holonp::IRFFTFactory factory;
};
...
TEST_F(IRFFTOracleTest, CF32Axis1) {
  const TDesc d    = device_desc({2, 4}, DType::CF32);
  const auto  data = as_bytes(std::vector<CF32>{{21.f, 0.f},
                                                {-2.f, 2.f},
                                                {-2.f, 0.f},
                                                {-2.f, -2.f},
                                                {8.f, 0.f},
...
TEST_F(IRFFTOracleTest, CF32Axis1ExplicitOddLength) {
  const TDesc d    = device_desc({2, 4}, DType::CF32);
  const auto  data = as_bytes(std::vector<CF32>{{21.f, 0.f},
                                                {-2.f, 2.f},
                                                {-2.f, 0.f},
                                                ...
                                                {-2.f, 0.f},
                                                {-2.f, -2.f}});
  const auto  j    = nlohmann::json{{"axis", 1}, {"n", 5}, {"norm", "ortho"}};
  const auto  run  = holonp_test::run_sync_factory(factory, std::vector<TDesc>{d},
                                                   std::vector<std::vector<std::byte>>{data}, j);
