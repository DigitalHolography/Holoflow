...
class IRFFT2InferTest : public ::testing::Test {
protected:
  holonp::IRFFT2Factory factory;
};

TEST_F(IRFFT2InferTest, ExpandsLastDimAndOutputsReal) {
...
TEST_F(IRFFT2InferTest, RejectsUnsupportedAxes) {
  const std::vector<TDesc> in = {device_desc({2, 3, 4}, DType::CF32)};
...
TEST_F(IRFFT2OracleTest, CF32DefaultAxes) {
  ...
  const auto  data = as_bytes(
      std::vector<CF32>{{10.f, 0.f}, {1.f, -2.f}, {3.f, 0.f}, {8.f, 0.f}, {-1.f, 1.f}, {2.f, 0.f}});
...
TEST_F(IRFFT2UpdateTest, ReusesTaskWithSameConfig) {
  struct CF32 {
    float re, im;
  };
  const TDesc d    = device_desc({2, 3}, DType::CF32);
