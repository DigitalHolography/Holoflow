...
class IFFTInferTest : public ::testing::Test {
protected:
  holonp::IFFTFactory factory;
};

TEST_F(IFFTInferTest, ComplexInputOutputsComplexWithSameShape) {
...
TEST_F(IFFTInferTest, RejectsRealInput) {
  const std::vector<TDesc> in = {device_desc({2, 4}, DType::F32)};
  EXPECT_THROW(factory.infer(in, nlohmann::json::object()), std::invalid_argument);
}
...
class IFFTOracleTest : public ::testing::Test {
protected:
  holonp::IFFTFactory factory;
};

TEST_F(IFFTOracleTest, CF32Axis1) {
...
TEST_F(IFFTOracleTest, CF32Axis1OrthoNorm) {
  const TDesc d = device_desc({2, 4}, DType::CF32);
  struct CF32 {
    float re, im;
  };
  const auto data = as_bytes(std::vector<CF32>{{1.f, 0.f},
                                               ...
                                               {2.f, 1.f},
                                               {-3.f, 2.f}});
  const auto j    = nlohmann::json{{"axis", 1}, {"norm", "ortho"}};
  const auto run  = holonp_test::run_sync_factory(factory, std::vector<TDesc>{d},
                                                  std::vector<std::vector<std::byte>>{data}, j);
