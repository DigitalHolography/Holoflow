...
static nlohmann::json max_settings_axis(int axis, bool keepdims = false) {
  return nlohmann::json{{"axis", axis}, {"keepdims", keepdims}};
}

static nlohmann::json max_settings_all(bool keepdims = false) {
  return nlohmann::json{{"axis", nullptr}, {"keepdims", keepdims}};
}

class MaxInferTest : public ::testing::Test {
protected:
  holonp::MaxFactory factory;
};

TEST_F(MaxInferTest, AxisReductionShape) {
  const std::vector<TDesc> in = {device_desc({2, 3}, DType::F32)};
  const auto               r  = factory.infer(in, max_settings_axis(1));
...
TEST_F(MaxInferTest, KeepdimsAllAxes) {
  const std::vector<TDesc> in = {device_desc({2, 3}, DType::U16)};
  const auto               r  = factory.infer(in, max_settings_all(true));
...
TEST_F(MaxUpdateTest, ReusesTaskWithSameConfig) {
  const TDesc d    = device_desc({2, 3}, DType::F32);
  const auto  data = as_bytes(std::vector<float>{3.f, 1.f, 2.f, 8.f, 5.f, 7.f});
  const auto  j    = max_settings_axis(1);
