...
auto make_timed_sync_creator(
    std::vector<std::chrono::steady_clock::time_point>& exec_times,
    std::mutex& timing_mutex
) -> std::function<std::unique_ptr<StubSyncTask>(
        std::span<const core::TDesc>,
        const nlohmann::json&,
        const core::SyncCreateCtx&
    )>
{
    return [&exec_times, &timing_mutex](
        std::span<const core::TDesc>, const nlohmann::json&, const core::SyncCreateCtx& ctx
    ) -> std::unique_ptr<StubSyncTask> {
...
auto make_timed_async_creator(
    ...
    std::atomic<bool>& has_data
        ...
        std::span<const core::TDesc>,
        const nlohmann::json&,
        const core::AsyncCreateCtx&
    )>
{
    return [&push_times, &pop_times, &timing_mutex, &has_data](
        std::span<const core::TDesc>, const nlohmann::json&, const core::AsyncCreateCtx& ctx
    ) -> std::unique_ptr<StubAsyncTask> {
...
TEST(Scheduler, StreamSynchronization) {
    ...
    process_spec.create = make_timed_async_creator(
        process_push_times, process_pop_times, timing_mutex, process_has_data);
    auto *process_factory = new RecordingAsyncFactory(std::move(process_spec));
    registry.register_async("process", std::unique_ptr<RecordingAsyncFactory>(process_factory));
    ...
    auto push_delay = std::chrono::duration_cast<std::chrono::nanoseconds>(
    ...
    auto sink_delay = std::chrono::duration_cast<std::chrono::nanoseconds>(
