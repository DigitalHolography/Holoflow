...
public:
  ...
  void enqueue(holovibes::ui::SignalSample sample) {
  ...
  void configure(double time_window_seconds) {
    if (widget_.isNull()) {
      return;
    }
    ...
    QMetaObject::invokeMethod(
        widget_.data(),
        [safe_widget, time_window_seconds]() {
...
private:
  void drain() {
    std::vector<holovibes::ui::SignalSample> samples;
    {
      std::lock_guard lock(mutex_);
  ...
  std::mutex                                    mutex_;
  std::vector<holovibes::ui::SignalSample>      pending_;
...
public:
  DisplaySignalHistoryTask(DisplaySignalHistorySettings settings, holoflow::core::TDesc idesc,
    ...
    dispatcher_->configure(settings_.time_window_seconds);
  }
  ...
  holoflow::core::OpResult execute(holoflow::core::SyncCtx &ctx) override {
    ...
    case holoflow::core::MemLoc::Device:
      CUDA_CHECK(cudaMemcpyAsync(&value, src, sizeof(value), cudaMemcpyDeviceToHost, stream_));
      CUDA_CHECK(cudaStreamSynchronize(stream_));
      break;
    default:
      throw std::logic_error("Unsupported memory location for scalar signal history");
    }
    ...
    // distorting the acquisition/pipeline timeline represented by consecutive a4 results.
    const double sample_time =
        static_cast<double>(valid_sample_index_++) * settings_.sample_time_seconds;
    dispatcher_->enqueue({sample_time, static_cast<double>(value)});
    return holoflow::core::OpResult::Ok;
  }
...
DisplaySignalHistoryFactory::infer(std::span<const holoflow::core::TDesc> input_descs,
  ...
  check(desc.shape[0] == 1, "input tensor must contain exactly one scalar");
