...
public:
  ...
  FftFrequencyToolDialog(double sampling_frequency_hz, int window_size, double start_frequency_hz,
    ...
    sampling_frequency_spin_->setToolTip(
        tr("The sampling frequency of the input signal. This is always set to the input sampling frequency."));
...
void MainWindow::refresh_visualization_availability() {
  ...
  const bool has_zernike_order =
      autofocus->is_z2_enabled() || autofocus->is_z3_enabled() || autofocus->is_z4_enabled() ||
  ...
  const auto zernike_order_message =
      tr("Select at least one Zernike coefficient to generate the metrics plot.");
  const auto xcorr_message =
      tr("Cross-correlation is available only in Single reference slope recovery mode.");
  const auto spectrum_message =
      tr("Spectrum visualizations are not generated by the current pipeline.");
  const auto cuts_message =
      tr("3D cut visualizations are not currently supported by the pipeline.");

  display_workspace_->set_visualization_content_expectations({
      {"xy_processed", {true, {}}},
      {"zernike_a4",
       {autofocus_enabled && has_zernike_order,
        autofocus_enabled ? zernike_order_message : autofocus_message}},
      {"zernike_phase", {autofocus_enabled, autofocus_message}},
      {"shack_hartmann", {autofocus_enabled, autofocus_message}},
...
void MainWindow::restore_persistent_state() {
  ...
  std::optional<bool> legacy_3d_cuts;
  std::optional<bool> legacy_zernike_metrics;
  std::optional<bool> legacy_zernike_phase;
  std::optional<bool> legacy_shack_hartmann;
  std::optional<bool> legacy_shack_hartmann_xcorr;
  ...
  if (settings.contains("cuts_3d")) {
    legacy_3d_cuts = settings.value("cuts_3d").toBool();
  }
  if (settings.contains("raw_view")) {
    legacy_raw_view = settings.value("raw_view").toBool();
  }
  if (settings.contains("raw_spectrum")) {
    legacy_raw_spectrum = settings.value("raw_spectrum").toBool();
  }
  if (settings.contains("processed_spectrum")) {
    legacy_processed_spectrum = settings.value("processed_spectrum").toBool();
  }
  ...
  if (settings.contains("show_zernike_metrics")) {
    legacy_zernike_metrics = settings.value("show_zernike_metrics").toBool();
  }
  if (settings.contains("show_phase")) {
    legacy_zernike_phase = settings.value("show_phase").toBool();
  }
  if (settings.contains("show_shack_hartmann")) {
    legacy_shack_hartmann = settings.value("show_shack_hartmann").toBool();
  }
  if (settings.contains("show_xcorr")) {
    legacy_shack_hartmann_xcorr = settings.value("show_xcorr").toBool();
  }
  ...
  const auto migrate_legacy_view_preference = [this, &settings](const QString      &id,
                                                                std::optional<bool> legacy_value) {
    if (!legacy_value.has_value()) {
      return;
    }

    const QString workspace_key =
        QStringLiteral("visualization_workspace/view_preferences/%1").arg(id);
    if (!settings.contains(workspace_key)) {
      display_workspace_->set_visualization_enabled(id, *legacy_value);
    }
  };
  migrate_legacy_view_preference(QStringLiteral("xy_raw"), legacy_raw_view);
  migrate_legacy_view_preference(QStringLiteral("raw_spectrum"), legacy_raw_spectrum);
  migrate_legacy_view_preference(QStringLiteral("processed_spectrum"), legacy_processed_spectrum);
  migrate_legacy_view_preference(QStringLiteral("xz_processed"), legacy_3d_cuts);
  migrate_legacy_view_preference(QStringLiteral("yz_processed"), legacy_3d_cuts);
  migrate_legacy_view_preference(QStringLiteral("zernike_a4"), legacy_zernike_metrics);
  migrate_legacy_view_preference(QStringLiteral("zernike_phase"), legacy_zernike_phase);
  migrate_legacy_view_preference(QStringLiteral("shack_hartmann"), legacy_shack_hartmann);
  migrate_legacy_view_preference(QStringLiteral("shack_hartmann_xcorr"),
                                 legacy_shack_hartmann_xcorr);
...
void MainWindow::initialize_display_widgets() {
  ...
  display_workspace_->register_visualization(
      {"xy_raw", "XY Raw", xy_raw_widget_, "xy_processed", DockPlacement::Tab, false});
  display_workspace_->register_visualization({"raw_spectrum", "Raw Spectrum", raw_spectrum_widget_,
                                              "zernike_phase", DockPlacement::Tab, false});
  ...
  display_workspace_->register_visualization({"xz_processed", "XZ Processed", xz_processed_widget_,
...
void MainWindow::show_fft_frequency_tool() {
  const double sampling_frequency_hz =
      last_input_fps_ > 0.0 ? last_input_fps_ : import_widget_->get_sampling_frequency_hz();
...
void MainWindow::on_import_start_clicked() {
  ...
  import_widget_->set_start_enabled(false);
  import_widget_->set_stop_enabled(false);
  display_workspace_->reset_visualization_content();
  refresh_command_bar();
...
void MainWindow::setup_validation_connections() {
  ...
  connect(display_workspace_, &VisualizationWorkspace::visualization_preferences_changed, this, cb);
...
pipeline::Settings MainWindow::get_pipeline_settings() {
  ...
  // View Settings
  {
    ...
    const auto view_enabled = [this](const QString &id, bool fallback) {
      return display_workspace_ == nullptr ? fallback
                                           : display_workspace_->is_visualization_enabled(id);
    };

    s.view_3d_cuts              = false;
    s.raw_view                  = view_enabled(QStringLiteral("xy_raw"), false);
    s.moment_type               = moment_from_str.at(view_widget_->get_image_type().toStdString());
    s.view_raw_spectrum         = view_enabled(QStringLiteral("raw_spectrum"), false);
    s.view_processed_spectrum   = view_enabled(QStringLiteral("processed_spectrum"), false);
    s.view_zernike_metrics      = view_enabled(QStringLiteral("zernike_a4"), true);
    s.view_zernike_phase        = view_enabled(QStringLiteral("zernike_phase"), false);
    s.view_shack_hartmann       = view_enabled(QStringLiteral("shack_hartmann"), false);
    s.view_shack_hartmann_xcorr = view_enabled(QStringLiteral("shack_hartmann_xcorr"), false);
  }
...
void MainWindow::set_pipeline_settings(const pipeline::Settings &s) {
  ...
  // --- View Settings ---
  {
    if (display_workspace_ != nullptr) {
      display_workspace_->set_visualization_enabled(QStringLiteral("xy_raw"), s.raw_view);
      display_workspace_->set_visualization_enabled(QStringLiteral("raw_spectrum"),
                                                    s.view_raw_spectrum);
      display_workspace_->set_visualization_enabled(QStringLiteral("processed_spectrum"),
                                                    s.view_processed_spectrum);
      display_workspace_->set_visualization_enabled(QStringLiteral("zernike_a4"),
                                                    s.view_zernike_metrics);
      display_workspace_->set_visualization_enabled(QStringLiteral("zernike_phase"),
                                                    s.view_zernike_phase);
      display_workspace_->set_visualization_enabled(QStringLiteral("shack_hartmann"),
                                                    s.view_shack_hartmann);
      display_workspace_->set_visualization_enabled(QStringLiteral("shack_hartmann_xcorr"),
                                                    s.view_shack_hartmann_xcorr);
      display_workspace_->set_visualization_enabled(QStringLiteral("xz_processed"), s.view_3d_cuts);
      display_workspace_->set_visualization_enabled(QStringLiteral("yz_processed"), s.view_3d_cuts);
    }
  }
