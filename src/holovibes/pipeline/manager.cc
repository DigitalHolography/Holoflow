...
void Manager::update_import_file(const QString &file_path) {
  ...
  // auto source = find_ids_by_tag("SOURCE");
  // DH_CHECK(source.size() <= 1);

  // if (source.size() == 1) {
  //   remove_id_from_tag_map(source.at(0));
  //   auto v = find_node_by_id(source.at(0));
  //   DH_CHECK(v);
  //   desc_graph_[*v].type = "Holofile";
  //   desc_graph_[*v].id = "holofile_source";
  //   desc_graph_[*v].config = config;
  // } else {
  //   auto new_v = boost::add_vertex(desc_graph_);
  //   desc_graph_[new_v].type = "Holofile";
  //   desc_graph_[new_v].id = "holofile_source";
  //   desc_graph_[new_v].config = config;
  //   node_tags_["holofile_source"].push_back("SOURCE");
  // }

  // auto v_source = find_node_by_id("holofile_source");

  // // TODO: Support intermediate nodes.
  // auto v_sink = find_node_by_id("processed_display_sink");
}
...
holoflow::model::DescriptorVertex Manager::add_input_queue_node() {
  ...
  auto nb_slots = TARGET_SIZE / *batch_size_;
  json config;
