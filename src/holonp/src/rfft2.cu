...
void to_json(nlohmann::json &j, const RFFT2Settings &s) {
  j = nlohmann::json{{"axes", s.axes}, {"norm", s.norm}};
}

void from_json(const nlohmann::json &j, RFFT2Settings &s) {
  if (j.contains("axes"))
...
inline void check(bool cond, const std::string &msg) {
  if (!cond)
    throw std::invalid_argument("RFFT2: " + msg);
}
...
inline float forward_scale(FftNorm norm, size_t n_fft) {
...
class RFFT2 : public holoflow::core::ISyncTask {
public:
  RFFT2(RFFT2Settings settings, holoflow::core::TDesc idesc, curaii::CufftHandle &&plan,
  ...
  const RFFT2Settings         &settings() const { return settings_; }
  void                         update_stream(cudaStream_t stream);

private:
  RFFT2Settings         settings_;
...
holoflow::core::OpResult RFFT2::execute(holoflow::core::SyncCtx &ctx) {
  ...
  for (size_t i = 0; i < input_offsets_.size(); ++i) {
    auto *in_ptr  = reinterpret_cast<float *>(idata_base + input_offsets_[i]);
    auto *out_ptr = reinterpret_cast<cuFloatComplex *>(odata_base + i * output_stride_bytes_);
    CUFFT_CHECK(cufftXtExec(plan_.get(), in_ptr, out_ptr, CUFFT_FORWARD));
  }

  const float scale = forward_scale(settings_.norm, n_fft_elems_);
  if (scale != 1.0f) {
...
holoflow::core::InferResult RFFT2Factory::infer(std::span<const holoflow::core::TDesc> input_descs,
                                                const nlohmann::json &jsettings) const {
  const auto settings = jsettings.get<RFFT2Settings>();
  check(input_descs.size() == 1, "expected 1 input");

  const auto &idesc = input_descs[0];
  check(idesc.mem_loc == holoflow::core::MemLoc::Device, "device memory only");
  check(idesc.dtype == holoflow::core::DType::F32, "input must be F32");
  ...
  const size_t w = idesc.shape[static_cast<size_t>(axes[1])];
  check(h > 0 && w > 0, "invalid FFT shape");

  auto out_shape                          = idesc.shape;
  out_shape[static_cast<size_t>(axes[1])] = w / 2 + 1;
...
RFFT2Factory::create(std::span<const holoflow::core::TDesc> input_descs,
  ...
  const size_t h             = idesc.shape[static_cast<size_t>(axes[0])];
  const size_t w             = idesc.shape[static_cast<size_t>(axes[1])];
  const size_t w_out         = odesc.shape[static_cast<size_t>(axes[1])];
  const size_t n_fft         = h * w;
  const size_t n_out         = h * w_out;
  ...
  const int istride =
      static_cast<int>(strides_bytes[axes[1]] / esize); // elements between consecutive W samples
  const int inembed_w =
      static_cast<int>(strides_bytes[axes[0]] / strides_bytes[axes[1]]); // W pitch

  long long idist         = static_cast<long long>(n_fft);
  ...
  check(n_out <= static_cast<size_t>(ll_max), "output stride exceeds cuFFT limits");
  ...
  const size_t output_stride_bytes = inner_batches * n_out * sizeof(cuFloatComplex);
  ...
  long long onembed[2] = {static_cast<long long>(h), static_cast<long long>(w_out)};

  CUFFT_CHECK(cufftXtMakePlanMany(plan.get(), 2, n, inembed, istride, idist, CUDA_R_32F, onembed, 1,
...
RFFT2Factory::update(std::unique_ptr<holoflow::core::ISyncTask> old_task,
  ...
  if (old_rfft != nullptr && input_descs.size() == 1) {
    ...
    const auto &old_idesc    = old_rfft->idesc();

    bool can_reuse = (new_settings == old_rfft->settings()) && same_desc(new_idesc, old_idesc);
