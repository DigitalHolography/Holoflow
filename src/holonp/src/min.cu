...
public:
  Min(MinSettings settings, holoflow::core::TDesc idesc, cudaStream_t stream, size_t total_out,
  ...
  const MinSettings           &settings() const { return settings_; }
...
#define LAUNCH_MIN_KERNEL(Type)                                                                    \
  ...
  case holoflow::core::DType::U8:
    LAUNCH_MIN_KERNEL(std::uint8_t);
    break;
  case holoflow::core::DType::U16:
    LAUNCH_MIN_KERNEL(std::uint16_t);
    break;
  case holoflow::core::DType::F32:
    LAUNCH_MIN_KERNEL(float);
    break;
  case holoflow::core::DType::CF32:
    LAUNCH_MIN_KERNEL(cuFloatComplex);
    break;
  default:
...
holoflow::core::InferResult MinFactory::infer(std::span<const holoflow::core::TDesc> input_descs,
  ...
  const auto odesc = utils::make_contiguous_desc(out_shape, idesc.dtype, idesc.mem_loc);
...
std::unique_ptr<holoflow::core::ISyncTask>
MinFactory::update(std::unique_ptr<holoflow::core::ISyncTask> old_task,
                   std::span<const holoflow::core::TDesc>     input_descs,
                   const nlohmann::json                      &jsettings,
                   const holoflow::core::SyncCreateCtx       &ctx) const {
  (void)infer(input_descs, jsettings);

  auto *old_min = dynamic_cast<Min *>(old_task.get());
  if (old_min != nullptr && input_descs.size() == 1) {
    const auto settings = jsettings.get<MinSettings>();
    if (settings == old_min->settings() && utils::same_desc(input_descs[0], old_min->idesc())) {
      old_min->update_stream(ctx.stream);
      return old_task;
    }
  }
