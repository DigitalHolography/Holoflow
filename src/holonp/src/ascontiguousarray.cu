...
class AsContiguousArray : public holoflow::core::ISyncTask {
public:
  AsContiguousArray() = default;
  AsContiguousArray(size_t ndim, size_t total_elems, size_t elem_size,
      ...
      : is_noop_(false), idesc_(std::move(idesc)), ndim_(ndim), total_elems_(total_elems),
  ...
  void                         set_idesc(holoflow::core::TDesc idesc) { idesc_ = std::move(idesc); }
...
std::unique_ptr<holoflow::core::ISyncTask>
AsContiguousArrayFactory::update(std::unique_ptr<holoflow::core::ISyncTask> old_task,
                                 std::span<const holoflow::core::TDesc>     input_descs,
                                 const nlohmann::json                      &jsettings,
                                 const holoflow::core::SyncCreateCtx       &ctx) const {
  (void)infer(input_descs, jsettings);

  auto *old_task_ptr = dynamic_cast<AsContiguousArray *>(old_task.get());
  if (old_task_ptr == nullptr || input_descs.size() != 1) {
