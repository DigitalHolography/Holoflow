...
subtract_kernel(const T *__restrict__ a, const T *__restrict__ b, T *__restrict__ out,
  ...
  if constexpr (std::is_same_v<T, cuFloatComplex>) {
    out[idx] = cuCsubf(a[a_off], b[b_off]);
  } else {
...
SubtractFactory::update(std::unique_ptr<holoflow::core::ISyncTask> old_task,
  ...
  if (utils::same_desc(input_descs[0], old_sub->input_descs()[0]) &&
      utils::same_desc(input_descs[1], old_sub->input_descs()[1])) {
    old_sub->update_stream(ctx.stream);
    return old_task;
  }
