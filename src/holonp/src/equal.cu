...
template <typename T>
__global__ void
equal_kernel(const T *__restrict__ a, const T *__restrict__ b, uint8_t *__restrict__ out,
  ...
  if constexpr (std::is_same_v<T, cuFloatComplex>) {
    auto av = a[a_off];
    auto bv = b[b_off];
    eq      = (av.x == bv.x) && (av.y == bv.y);
  } else {
    eq = (a[a_off] == b[b_off]);
  }
  out[idx] = static_cast<uint8_t>(eq);
}

inline void check(bool cond, const std::string &msg) {
  if (!cond) {
    throw std::invalid_argument("Equal: " + msg);
  }
}
...
#define LAUNCH_TYPE(T)                                                                             \
  equal_kernel<T><<<grid, block, 0, stream_>>>(                                                    \
      (T *)ctx.inputs[0].data(), (T *)ctx.inputs[1].data(), (uint8_t *)ctx.outputs[0].data(),      \
...
holoflow::core::InferResult EqualFactory::infer(std::span<const holoflow::core::TDesc> inputs,
  ...
  holoflow::core::TDesc o(out_shape, holoflow::core::DType::U8, holoflow::core::MemLoc::Device);
...
EqualFactory::create(std::span<const holoflow::core::TDesc> inputs, const nlohmann::json &j,
...
EqualFactory::update(std::unique_ptr<holoflow::core::ISyncTask> old_task,
  ...
  if (utils::same_desc(input_descs[0], old_equal->input_descs()[0]) &&
      utils::same_desc(input_descs[1], old_equal->input_descs()[1])) {
