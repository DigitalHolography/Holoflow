...
template <typename T> __device__ __forceinline__ float as_f32(T v) {
  return static_cast<float>(v);
}

template <typename T, int BLOCK_SIZE>
__global__ void
reduce_group_minmax_kernel(const T *__restrict__ in, float *__restrict__ mins,
                           float *__restrict__ maxs, size_t total_groups, size_t total_red,
                           int group_ndim, int red_ndim, const size_t *__restrict__ in_strides,
...
apply_normalize_kernel(const T *__restrict__ in, float *__restrict__ out, size_t total_elems,
                       ...
                       const size_t *__restrict__ in_strides, const size_t *__restrict__ out_strides,
                       const int *__restrict__ group_axes, const size_t *__restrict__ group_strides,
                       const float *__restrict__ mins, const float *__restrict__ maxs, float lo,
                       float hi) {
  const size_t tid = static_cast<size_t>(blockIdx.x) * blockDim.x + static_cast<size_t>(threadIdx.x);
  if (tid >= total_elems) {
    return;
  }
...
Normalize::Normalize(const NormalizeSettings &settings, cudaStream_t stream, size_t ndim,
                     size_t group_ndim, size_t red_ndim, size_t total_elems, size_t total_groups,
                     size_t total_red, DevPtr<size_t> shape, DevPtr<size_t> in_strides,
    ...
    : settings_(settings), stream_(stream), ndim_(ndim), group_ndim_(group_ndim), red_ndim_(red_ndim),
      total_elems_(total_elems), total_groups_(total_groups), total_red_(total_red),
      d_shape_(std::move(shape)), d_in_strides_(std::move(in_strides)),
...
NormalizeFactory::create(std::span<const holoflow::core::TDesc> input_descs,
  ...
  return std::make_unique<Normalize>(
      settings, ctx.stream, idesc.shape.size(), plan.group_axes.size(), plan.red_axes.size(),
