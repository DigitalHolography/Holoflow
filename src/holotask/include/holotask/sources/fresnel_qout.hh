...
struct FresnelQoutSettings {
  ...
  bool operator==(const FresnelQoutSettings &other) const {
    return lambda == other.lambda && dx == other.dx && dy == other.dy && nx == other.nx &&
           ny == other.ny;
  }
};
...
public:
  ...
  const FresnelQoutSettings   &get_settings() const { return settings_; }
  void                         update_stream(cudaStream_t stream) { stream_ = stream; }

private:
  FresnelQout(const FresnelQoutSettings &settings, const holoflow::core::TDesc &idesc,
              DevPtr<float> &&d_r2, cudaStream_t stream);

  friend class FresnelQoutFactory;
...
class FresnelQoutFactory : public holoflow::core::ISyncTaskFactory {
public:
