...
Wrap2Pi::Wrap2Pi(const Wrap2PiSettings &settings, cudaStream_t stream)
    : settings_(settings), stream_(stream) {}

holoflow::core::OpResult Wrap2Pi::execute(holoflow::core::SyncCtx &ctx) {
...
Wrap2PiFactory::infer(std::span<const holoflow::core::TDesc> input_descs,
  ...
  auto settings = jsettings.get<Wrap2PiSettings>();
  (void)settings;
...
Wrap2PiFactory::create(std::span<const holoflow::core::TDesc> input_descs,
  ...
  auto *task = new Wrap2Pi(settings, ctx.stream);
